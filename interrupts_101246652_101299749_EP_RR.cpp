...
std::tuple<std::string> run_simulation(std::vector<PCB> list_processes) {
    ...
    while(true) {
        ...
        if(running.state == RUNNING && !ready_queue.empty()) {
            int highest_priority = ready_queue[0].PID;
            for(const auto &proc : ready_queue) {
                if(proc.PID < highest_priority) {
                    highest_priority = proc.PID;
                }
            }
            
            if(highest_priority < running.PID) {
        ...
        // 5. Schedule next process by priority
        if(running.state != RUNNING && !ready_queue.empty()) {
